...
export function FoodAgentView() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-orange-500 to-amber-400 flex items-center justify-center text-xl sm:text-2xl shadow-lg shadow-orange-500/25">
          🍜
        </div>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Food Agent</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">{uniqueDishes.length} dishes saved</p>
        </div>
      </motion.div>
      ...
      {loading ? (
        ...
        >
          {uniqueDishes.map((recipe) => (
            ...
            >
              {/* Image */}
              <div className="relative h-36 sm:h-44 bg-gradient-to-br from-orange-500/10 to-amber-400/10 overflow-hidden">
                {recipe.image_url ? (
                  <img
      ...
      {/* ===== Frosted Glass Recipe Modal (Production-Grade) ===== */}
      <Dialog.Root open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
        <AnimatePresence>
          {selected && (
            <Dialog.Portal forceMount>
              {/* Fullscreen Overlay — z-[100] to always float above navbar (z-50) */}
              ...
              {/* Content — fixed centering with inset-0 flex wrapper */}
              <Dialog.Content asChild>
                ...
                >
                  {/* Actual modal card */}
                  <div className="pointer-events-auto w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-3xl glass-heavy shadow-2xl shadow-black/20">
                    {/* Modal header image */}
                    <div className="relative h-40 sm:h-48 bg-gradient-to-br from-orange-500/20 to-amber-400/20 overflow-hidden rounded-t-3xl">
                      ...
                      <div className="absolute bottom-4 left-4 sm:left-5 right-14">
                        <Dialog.Title className="text-white text-xl sm:text-2xl font-bold drop-shadow-lg leading-snug">
                          {versions[activeVersion]?.dish_name || selected.dish_name}
                        </Dialog.Title>
                        <Dialog.Description className="text-white/70 text-xs sm:text-sm mt-1">
                          Version {versions[activeVersion]?.version || 1}
                        </Dialog.Description>
                      </div>
                      <Dialog.Close asChild>
                        <button className="absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center text-white/80 hover:text-white hover:bg-black/50 transition-colors">
                    ...
                    {/* Version tabs */}
                    {versions.length > 1 && (
                      <div className="flex gap-2 px-4 sm:px-5 pt-4 overflow-x-auto scrollbar-none">
                        {versions.map((v, i) => (
                          <button
                            key={v.id}
